...
void run_gpu_color_test(PPM_IMG img_in)
{
    ...
    sdkStartTimer(&timer);
    img_obuf_hsl = contrast_enhancement_g_hsl(img_in);
    sdkStopTimer(&timer);
    ...
    sdkCreateTimer(&timer);
    sdkStartTimer(&timer);
    img_obuf_yuv = contrast_enhancement_g_yuv(img_in);
    sdkStopTimer(&timer);
...
PPM_IMG read_ppm(const char * path){
    ...
    cudaMallocHost((void**)&result.img_r,result.w * result.h * sizeof(unsigned char));
    cudaMallocHost((void**)&result.img_g,result.w * result.h * sizeof(unsigned char));
    cudaMallocHost((void**)&result.img_b,result.w * result.h * sizeof(unsigned char));
...
void free_ppm(PPM_IMG img)
{
    ...
    cudaFreeHost(img.img_r);
    cudaFreeHost(img.img_g);
    cudaFreeHost(img.img_b);
}
